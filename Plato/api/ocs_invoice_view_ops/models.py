from sqlalchemy import Column, VARCHAR, BigInteger, Date, Integer, Time, \
    ForeignKey, DateTime
...
class InvoiceItemVO(Base):
    ...
    def to_dict(self):
        invoices_item_dict = {
            "id": self.id,
            "rx_number": self.invoice_item_id,
            "qty": self.qty,
            "name": self.name,
            "unit": self.unit,
            "ddose": self.ddose,
...
class DrugDetailsVO(Base):
    ...
    def to_dict(self):
        drug_details_dict = {
            ...
            "admin_time": self.admin_time.strftime('%H:%M:%S'),
            "batch": self.batch,
            "admin_description": self.admin_description
        }
        return drug_details_dict
